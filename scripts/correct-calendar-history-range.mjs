...
export async function applyHistoricalCorrectionsByDateRange({
  ...
  if (!changes?.length)
    throw new Error(
      "No historical changes are available in the selected date range.",
    );
...
async function main() {
  ...
  console.log(
    `Historical range correction applied: ${results.length} event(s).`,
  );
}
...
if (isDirectExecution)
  main().catch((error) => {
    console.error(`Historical range correction rejected: ${error.message}`);
    process.exitCode = 1;
  });
