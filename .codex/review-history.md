...
SESSION_STATE:
  reviewed:
    ...
    - current h1-h4 hierarchy across Home, Galeria, Afiliados, Lightbox, events and Footer
    ...
    - responsive typography cascade for InfoCell labels in AfiliadosSection (base, lg and land-compact interaction)
  open_criticals:
  ...
  resolved_2026_07_11:
    ...
    - "[404] unknown production URLs returned HTTP 200 with homepage metadata. The legacy state reports a dedicated noindex 404 route and generated dist/404.html as the fix."
    - "[SSR] initial HTML content depended entirely on client JavaScript. The legacy state reports shared server rendering and prerendered route HTML as the fix."
    - "[SEO SOURCE] route metadata and JSON-LD were calculated independently. The legacy state reports a shared RouteSeoPayload as the fix."
    - "[SCROLL] ScrollToTop could interfere with fragment links. The legacy state reports preserving initial and hash navigation positions as the fix."
  resolved_2026_07_13:
    - "[RESPONSIVE] The mobile navbar menu button was reported to have a 44px interactive area."
    - "[RESPONSIVE] Gallery dot selectors were reported to use 44px buttons around their visual indicators."
  resolved_2026_07_16:
    - "[TAILWIND] The legacy state reports removal of no-effect utilities and equivalent padding/inset shorthands, with typecheck, build, and 1280x720 geometry checks passing."
    - "[DESIGN] The legacy state records rounded-3xl as the primary-container convention with scoped exceptions for internal controls."
  resolved_2026_07_17:
    - "[RESPONSIVE] The legacy state reports affiliates pagination in groups of two with mobile scrolling and bounded desktop presentation."
  pending_review:
