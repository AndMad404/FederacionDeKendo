...
test("uses approved input and 4K limits", () => {
  assert.deepEqual(EVENT_GALLERY_LIMITS.inputFormats, ["jpeg", "png", "webp", "avif"]);
...
test("accepts the minimum dimensions independently of orientation", async () => {
  const context = await fixture([{ name: "portrait.jpg", id: "portrait", buffer: await image("red", 320, 480) }]);
  try {
...
test("valid public album freezes five naturally ordered sanitized responsive images and warns about the sixth", async () => {
  const names = ["photo10.jpg", "photo2.jpg", "photo1.jpg", "photo5.jpg", "photo4.jpg", "photo3.jpg"];
  const colors = ["red", "green", "blue", "yellow", "cyan", "magenta"];
  const files = await Promise.all(names.map(async (name, index) => ({ name, id: `private-${index}`, buffer: await image(colors[index]) })));
  const context = await fixture(files);
  try {
    const result = await run(context.options);
    assert.equal(result.galleries["2026-01-01-evento"].images.length, 5);
    assert.equal(result.warnings.some((warning) => warning.includes("additional files")), true);
...
test("false MIME, dimensions, weight, duplicates and partial downloads are warned and never replace prior output", async () => {
  ...
  try {
    ...
    const cases = [
      ...
      [{ name: "small.jpg", id: "small", buffer: await image("red", 320, 240) }],
      [{ name: "large.jpg", id: "large", buffer: Buffer.alloc(EVENT_GALLERY_LIMITS.maxBytes + 1) }],
      [{ name: "oversize.jpg", id: "oversize", buffer: await image("red", 3841, 2160) }],
      [{ name: "ok.jpg", id: "ok", buffer: await image("green") }, { name: "broken.jpg", id: "broken" }],
    ];
    ...
    try {
      ...
      assert.equal(result.galleries["2026-01-01-evento"].images.length, 1);
      assert.equal(result.warnings.some((warning) => warning.includes("duplicate ignored")), true);
    } finally {
