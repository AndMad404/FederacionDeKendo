...
test("calendar card uses native sharing when available", async ({ page }) => {
  await openCalendar(page);
  await page.evaluate(() => {
  ...
  await page
    .getByRole("button", { name: /Compartir Examen por WhatsApp o copiar enlace/ })
    .click();

  const sharedData = await page.evaluate(
    () => (window as Window & { sharedData?: ShareData }).sharedData,
  );
  expect(sharedData?.title).toBe("Examen");
  expect(sharedData?.url).toContain("/eventos/2026-08-08-examen/");
});

test("calendar card copies its canonical link and clears feedback", async ({
  context,
  page,
}) => {
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole("button", { name: "Enlace de Examen copiado" }),
  ).toBeVisible();
  expect(await page.evaluate(() => navigator.clipboard.readText())).toContain(
    "/eventos/2026-08-08-examen/",
  );

  await page.waitForTimeout(2300);
  await expect(
    page.getByRole("button", { name: /Compartir Examen por WhatsApp o copiar enlace/ }),
  ).toBeVisible();
});

test("calendar card falls back to WhatsApp when clipboard copying fails", async ({
  page,
}) => {
  await openCalendar(page);
  await page.evaluate(() => {
