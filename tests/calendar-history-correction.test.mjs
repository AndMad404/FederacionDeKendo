...
async function fixture(report = createReport()) {
  ...
  const other = { ...published, sourceId: "other-source", slug: "2025-01-01-other", title: "Other" };
...
async function run(files, fields, overrides = {}) {
  const change = JSON.parse(await readFile(files.reportPath, "utf8")).historicalChanges[0];
...
test("C3 accepts one field and preserves every unselected field and event", async () => {
  ...
  try {
    ...
    assert.deepEqual(registry.events[0], { ...published, title: proposed.title });
...
test("C3 preserves the old slug as an alias", async () => {
  ...
  try {
    ...
    const event = JSON.parse(await readFile(files.registryPath, "utf8")).events[0];
    ...
    assert.match(generated, new RegExp(`aliases: \\["2026-01-10-anterior","${published.slug}"\\]`));
  } finally { await rm(files.directory, { recursive: true, force: true }); }
});
...
test("C3 rejects disappeared_del_feed and emits no private values", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(serialized, /drive\.google\.com|ALBUM_FOTOS|CALENDAR_ICS_URL/);
...
test("fingerprints ignore JSON property order", () => {
  ...
  assert.equal(fingerprintHistoricalSnapshot(reversed), fingerprintHistoricalSnapshot(published));
  const report = createReport().historicalChanges[0];
  assert.equal(
    fingerprintHistoricalProposal(report.sourceId, [...report.differences].reverse()),
...
test("later synchronization still reports differences not accepted", async () => {
  ...
  try {
    await run(files, ["title"]);
    const registry = JSON.parse(await readFile(files.registryPath, "utf8"));
    const report = detectHistoricalChanges(registry, [{ ...proposed, historical: undefined }], new Date("2026-03-02"));
    const remaining = report.historicalChanges.find(({ sourceId }) => sourceId === published.sourceId);
    assert.deepEqual(remaining.differences.map(({ field }) => field), ["slug", "location", "summary"]);
  } finally { await rm(files.directory, { recursive: true, force: true }); }
});
...
test("disappearance of every historical event aborts before producing a registry", () => {
  assert.throws(
    () => mergeRegistry({ version: 3, events: [published] }, [], new Date("2026-03-01")),
